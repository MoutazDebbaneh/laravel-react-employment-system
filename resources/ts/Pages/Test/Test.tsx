...
export default function Test({ locale, translations, categories, auth }: {
    locale: Locale,
                ...
                <section className='top-section mt-[4.6rem] max-w-full mx-10 lg:mx-0 grid grid-cols-1 2xl:grid-cols-2'>
                    <div className="left-section">
                        ...
                        <div className="quick-search">
                            <div className="form-rect w-[40rem] bg-white rounded-lg my-10 shadow-xl h-16 flex items-center px-4">
                                <span className={`z-10 ${locale == Locale.English ? 'mr-[-5px]' : 'ml-[-5px]'}`}>
                                ...
                                <select onChange={(e) => setCategory(e.target.value)} value={category} className="w-56 block h-[50px] border-none text-[#A0ABB8] focus:text-black focus:shadow-none focus:ring-0 text-[15px]" id="role" name="role">
                                    {categories.map(c => <option key={c.id} value={c['name_en']} >{c['name_en']}</option>)}
                                </select>
                                <span className={`z-10 ${locale == Locale.English ? '-mr-2' : '-ml-2'}`}>
                                ...
                                />
                                <span>
                                    <a href="#" className='bg-primary-blue px-[25px] py-[10px] rounded-lg me-2 text-white hover:bg-dark-blue block hover:-translate-y-0.5 transition-all'>Search</a>
                                </span>
                            </div>
                        </div>
                    </div>
